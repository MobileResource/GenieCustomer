...
- (void)viewDidLoad
{
    ...
    self.title = [self.dict objectForKey:@"pv_name"];
    ...
    [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:[UtilitiesHelper getFullImageURL:[self.dict objectForKey:@"pv_image"]]] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
    ...
    self.senderId = [GVUserDefaults standardUserDefaults].csId;
    self.senderDisplayName = [NSString stringWithFormat:@"%@",[GVUserDefaults standardUserDefaults].csName];
...
-(BOOL)receiveMessage:(NSDictionary*)dict{
    NSString * pv_id = [dict objectForKey:@"pv_id"];
    if ([pv_id isEqualToString:[self.dict objectForKey:@"pv_id"]]) {
        
        NSString * senderId = @"";
        
        senderId = [NSString stringWithFormat:@"%@", dict[@"pv_id"]];
        
        NSString * dateString = dict[@"created_at"];
        NSDate * date = [UtilitiesHelper convert2Date:dateString];
        
        JSQMessage * message = [[JSQMessage alloc] initWithSenderId:[NSString stringWithFormat:@"%@",[dict objectForKey:@"pv_id"]] senderDisplayName:[dict objectForKey:@"pv_name"] date:date text:dict[@"message"]];
...
-(void)loadMessages{
    NSDictionary * param = @{@"pv_id": [self.dict objectForKey:@"pv_id"],
                             @"cs_id":[GVUserDefaults standardUserDefaults].csId,
                             @"category_id" : [self.dict objectForKey:@"ct_id"]};
    
    [Customer getMessagesParameters:param withSuccessBlock:^(NSArray *response) {
        for (NSDictionary *item in response){
            ...
            if ([senderId isEqualToString:[GVUserDefaults standardUserDefaults].csId]) {
                JSQMessage * message = [[JSQMessage alloc] initWithSenderId:[GVUserDefaults standardUserDefaults].csId senderDisplayName:[GVUserDefaults standardUserDefaults].csName date:date text:item[@"message"]];
                [messages addObject:message];
            } else {
                JSQMessage * message = [[JSQMessage alloc] initWithSenderId:[NSString stringWithFormat:@"%@",[self.dict objectForKey:@"pv_id"]] senderDisplayName:[self.dict objectForKey:@"pv_name"] date:date text:item[@"message"]];
                [messages addObject:message];
            }
        }
...
-(void)sendMessage:(NSDictionary*) msg
{
    ...
    [Customer sendMessageWithParameters:msg withSuccessBlock:^(NSDictionary *response) {
...
//    NSAssert(NO, @"Error! required method not implemented in subclass. Need to implement %s", __PRETTY_FUNCTION__);
    ...
    item[@"cs_id"]     = [GVUserDefaults standardUserDefaults].csId;
    item[@"pv_id"]     = [self.dict objectForKey:@"pv_id"];
    item[@"ct_id"]     = [self.dict objectForKey:@"ct_id"];
    item[@"message"]    = text;
    
    JSQMessage * message = [[JSQMessage alloc] initWithSenderId:[GVUserDefaults standardUserDefaults].csId senderDisplayName:[GVUserDefaults standardUserDefaults].csName date:date text:text];
    [messages addObject:message];
